...
/**
  * SkBill  数据导入
  */
object FlumeSourceMain {
  val logger = LoggerFactory.getLogger(FlumeSourceMain.getClass)
  ...
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().setAppName("flume_" + app_code + "_" + event_code)
    // .setMaster("local[2]")
  ...
  def startStream(ssc: StreamingContext, kafkaParams: Map[String, Object], topicsSet: scala.collection.immutable.Set[String]): Unit = {
    ...
    logger.warn("app_code :" + app_code + " event_code: " + event_code)
    ...
    inputStream.foreachRDD(rddRaw => {
      ...
      //  过滤含有关键字的　日志
      val messagesRdd = rddRaw.map(message => {
        message.value()
      }).filter(message => {
        val item = JsonUtils.parseJson(message)
        val event_code_data = item.getString("event_code").trim
        event_code_data.equals(event_code_broadcast.value.trim)
      }).cache()
